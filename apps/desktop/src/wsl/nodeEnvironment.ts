...
const wslNodeEngineCheckMain = function wslNodeEngineCheckMain() {
...
function buildWslNodeEngineCheckScript(): string {
  return `${satisfiesSemverRange.toString()}
(${wslNodeEngineCheckMain.toString()})();`;
}
...
export const WSL_NODE_ENV_SCRIPT = `prepend_path_if_dir() {
  if [ -d "$1" ]; then
    case ":$PATH:" in
      *":$1:"*) ;;
      *) PATH="$1:$PATH" ;;
...
export function buildWslNodeEnvScript(input?: {
  readonly nodeEngineRange?: string | null;
}): string {
  return stripTrailingNewlines(
    applyScriptPlaceholders(WSL_NODE_ENV_SCRIPT, {
      T3_NODE_ENGINE_RANGE: shellSingleQuote(input?.nodeEngineRange?.trim() || ""),
      T3_NODE_ENGINE_CHECK_SCRIPT: stripTrailingNewlines(buildWslNodeEngineCheckScript()),
    }),
  );
}
