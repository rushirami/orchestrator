...
Human revision requests wait until every agent has settled. Rework-result handling marks its own stage complete before checking that no other agent is active. Project deletion reads the final workflow records after the project command succeeds, including templates saved while deletion was in progress.
